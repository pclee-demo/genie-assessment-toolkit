...
bm_suggestion_count = len(benchmark_suggestions) if "benchmark_suggestions" in dir() else 0
a6_flags = []
if bm_count == 0:
    if bm_suggestion_count > 0:
        a6_flags.append(
            f"No confirmed benchmarks — {bm_suggestion_count} benchmark suggestion(s) were auto-generated by Databricks "
            f"but not yet confirmed. Go to Configuration > Benchmarks, review the suggestions, add verified SQL answers, "
            f"and confirm them to activate benchmarking."
        )
    else:
